...
<form id="updateRingLabel-{labelUuid}" bind:this={formElement} action="?/updateLabel" method="POST" on:submit|preventDefault={handleSubmit}>
    <div role="form" class="grid grid-rows-2 gap-2 items-center align-center p-1 opacity-{labelEditOpacity}" on:mouseover={() => labelEditOpacity = DEFAULT_HOVER_OPACITY} on:mouseout={() => labelEditOpacity = DEFAULT_NON_HOVER_OPACITY} on:focus={() => labelEditOpacity = DEFAULT_HOVER_OPACITY} on:blur={() => labelEditOpacity = DEFAULT_NON_HOVER_OPACITY}>
