...
<div id={`deleteRing-${id}`}>
    <div class="flex items-center align-center w-full pr-4 truncate">
            ...
            <form id="deleteRingForm-{id}" bind:this={formElement} action="?/deleteSessionFile" method="POST" use:enhance={({ formData }) => {
                ...
                return async ({ result }) => {
                    ...
                    if (result.type === "success"){
                        if (updateVerbose) triggerToast("File deleted sucessfully", ToastType.SUCCESS, toastStore);
                    } else {
