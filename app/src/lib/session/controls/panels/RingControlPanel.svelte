<script lang="ts">
    ...
    function handleTitleUpdate(event: any, id: string) {
        changeRingTitle(id, event.detail.newTitle)
    }
    ...
    function handleColorPickerChange(event: Event, ring: Ring) {
        ...
        if (target) changeRingColor(ring.id, target.value)
    }
                    ...
                    {#each $ringData as ring}
                        <div class="grid grid-cols-8 gap-x-2 items-center align-center p-2 rounded-token hover:variant-soft hover:cursor-pointer">
                            <div class="flex items-center gap-x-2 col-span-7">
                                ...
                                <div class="mt-0.5">
                                    <PalettePopup id={`ringPalette-${ring.index}`} on:selectColor={(event) => changeRingColor(ring.id, event.detail.color)} color={ring.color}></PalettePopup>
                                </div>
                                
                                {#if ring.visible}
                                    <button class="btn btn-icon h-4 w-4 mr-4" on:click={() => toggleRingVisibility(ring.id)}>
                                ...
                                <EditableTitle title={ring.title} titleColor={ring.color} on:update={(event) => handleTitleUpdate(event, ring.id)} />
                            </div>
                            <div class="flex justify-end gap-x-2 col-span-1">
                                {#if ring.type !== RingType.LABEL}
                                    ...
                                    {/if}
                                {/if}
                                <button class="btn btn-icon h-4 w-4 ml-4 mr-4" on:click={() => removeRing(ring.id)}>
                                    <svg data-slot="icon" aria-hidden="true" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
