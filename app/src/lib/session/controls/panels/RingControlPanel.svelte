...
<div id="brickRingControlPanel" class="p-2 text-base">
    ...
    {#if showNewRingMenu}
        <div>
            ...
            <p class="opacity-60 my-2">Specialty rings</p>
            <div class="p-2">
                <div class="">
                    <button class="btn variant-outline-surface mr-2" disabled={selectedReference ? false : true} on:click={() => newRing = RingType.REFERENCE}>
                        <div class="flex items-center align-center">
        ...
        {#if $rings.length}
            <div>
                <p class="opacity-60 mb-2">Rings</p>
                    {#each $rings as ring}
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-8 gap-x-2 items-center align-center p-2 rounded-token hover:variant-soft hover:cursor-pointer">
                                ...
                                <div class="flex justify-end gap-x-2 col-span-1">
                                    ...
                                    <button class="btn btn-icon h-4 w-4 ml-4" on:click={() => removeRing(ring.index)}>
                                        <svg data-slot="icon" aria-hidden="true" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
