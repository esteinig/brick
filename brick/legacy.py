...
class RingGenerator:
    ...
    def brick(
        self, html_output: Path = Path.cwd() / "brick.html", json_output: Path = None
    ):
...
class AnnotationRing(Ring):
    ...
    def read_genbank(self, file: Path) -> None:
        ...
        features = [
            feature for feature in genome.features if feature.type == self.feature
        ]
        ...
        for feature in features:
            ...
            for qualifier in self.extract.keys():
                if qualifier in feature.qualifiers:
                    text_tuple = (
                        self.extract[qualifier],
                        "".join(feature.qualifiers[qualifier]),
                    )
                    qualifier_texts.append(text_tuple)
            qualifier_texts.insert(
                0,
                (
                    "Location: ",
    ...
    def read_comparison(self, file):
        ...
        with open(file, "r") as infile:
            ...
            for row in reader:
                ...
                if (
                    positions[1] - positions[0] >= self.min_length
                    and float(row[2]) >= self.min_identity
                ):
        ...
        texts = [
            [("Genome:", self.name), ("BLAST Identity:", str(v) + "%")]
    ...
    def _get_db(self):
        ...
        call(
            [
                "makeblastdb",
                "-in",
                os.path.join(os.getcwd(), self.reference),
                "-dbtype",
                self.type,
                "-out",
                self.name_db,
