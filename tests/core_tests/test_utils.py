...
def test_with_enough_space():
    disk_space_required_gb = (
        get_available_space(EXISTENT_PATH) / 1024**3 - 1
    )  # slightly less than available space
...
def test_with_insufficient_space():
    disk_space_required_gb = (
        get_available_space(EXISTENT_PATH) / 1024**3 + 1
    )  # slightly more than available space
...
def test_sanitize_input_mixed_content():
    input_string = (
        "<div onclick='alert(1)'>Hello {$ne: null}<svg><script></script></svg></div>"
    )
