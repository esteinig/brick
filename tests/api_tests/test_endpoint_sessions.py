...
# Mock session data
mock_session_data = Session(id="some_uuid", date="some_iso_date", files=[], rings=[])
...
def mock_motor_collection():
    ...
    )  # session query must be returned as dict
...
async def test_get_session_success(
    mock_get_session_collection_motor, mock_motor_collection
):
...
@patch("brick.api.endpoints.sessions.get_session_collection_motor")
@pytest.mark.asyncio
async def test_get_session_failure(
    mock_get_session_collection_motor, mock_motor_collection
):
    # Set the mock collection as the return value of the function
    mock_get_session_collection_motor.return_value = mock_motor_collection
