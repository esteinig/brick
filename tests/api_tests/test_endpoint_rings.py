...
def create_mock_session_directory(session_id: str = str(uuid.uuid4())) -> Path:
    tmp = Path(settings.WORK_DIRECTORY / session_id)
    tmp.mkdir(parents=True)
    return tmp

def create_mock_session_file(session_id: str = str(uuid.uuid4())) -> Path:
...
def mock_label_ring_data():
    ...
    mock_label_ring_tmpdir = create_mock_session_directory()
...
async def test_create_label_ring_success(mock_process_label_ring, mock_label_ring_data):
    ...
    mock_process_label_ring.delay.return_value = AsyncMock(id=mock_label_ring_response.task_id)
    
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.post("/rings/label", json=mock_label_ring_schema.model_dump())
...
def mock_annotation_ring_data():
    ...
    mock_annotation_ring_tmpdir = create_mock_session_directory()
...
async def test_create_annotation_ring_success(mock_process_annotation_ring, mock_annotation_ring_data):
    mock_annotation_ring_schema, mock_annotation_ring_response = mock_annotation_ring_data
    print(mock_annotation_ring_schema.session_id)
    mock_process_annotation_ring.delay.return_value = AsyncMock(id=mock_annotation_ring_response.task_id)

    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.post("/rings/annotation", json=mock_annotation_ring_schema.model_dump())
        assert response.status_code == 202
        assert response.json() == mock_annotation_ring_response.model_dump()

@pytest.mark.asyncio
@patch('brick.api.endpoints.rings.process_annotation_ring')
async def test_create_annotation_ring_failure(mock_process_annotation_ring, mock_annotation_ring_data):
...
def mock_blast_ring_data():
    ...
    mock_blast_ring_tmpdir = create_mock_session_directory()
...
async def test_create_blast_ring_success(mock_process_blast_ring, mock_blast_ring_data):
    ...
    mock_process_blast_ring.delay.return_value = AsyncMock(id=mock_blast_ring_response.task_id)

    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.post("/rings/blast", json=mock_blast_ring_schema.model_dump())
        assert response.status_code == 202
